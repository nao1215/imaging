...
`
	...
	body := buf.String()
	url := "https://github.com/go-spectest/imaging/issues/new?title=[Bug Report] Title&body=" + url.QueryEscape(body)

	if !openBrowser(url) {
		fmt.Print("Please file a new issue at https://github.com/go-spectest/imaging/issues/new using this template:\n\n")
		fmt.Print(body)
	}
